...
- Ir para `main`
- Fazer `pull` da `main`
- Abrir um branch novo no template: `<conventional-commit>/<service>/<subject>`
  - Ex.: `feat/user-service/foo`
  - Se não houver `service`, usar `project-name`

Regras:
- O `service` deve preferencialmente refletir o app/serviço (`user-service`, `payment-service`, etc.)
...
Título do PR: `<conventional-commit>(<service>): <simple_description>`
- Ex.: `feat(user-service): add foo processing`
