...
func TestFinishSessionUC_Execute(t *testing.T) {
	userID := uuid.New()
	sessionID := uuid.New()
	workoutID := uuid.New()
	...
	tests := []struct {
		name          string
		input         sessions.FinishSessionInput
		mockSetup     func(*mockFinishSessionRepo)
		expectedError error
	}{
		{
			name: "success - finishes active session",
			input: sessions.FinishSessionInput{
				UserID:    userID,
				SessionID: sessionID,
				Notes:     "Great session",
			},
			mockSetup: func(r *mockFinishSessionRepo) {
...
// mockFinishSessionRepo is a mock SessionRepository for FinishSession tests.
type mockFinishSessionRepo struct {
...
func (m *mockFinishSessionRepo) Create(ctx context.Context, session *entities.Session) error {
	return nil
}

func (m *mockFinishSessionRepo) FindActiveByUserID(ctx context.Context, userID uuid.UUID) (*entities.Session, error) {
	return nil, nil
}

func (m *mockFinishSessionRepo) FindByID(ctx context.Context, sessionID uuid.UUID) (*entities.Session, error) {
	if m.findByID != nil {
...
func (m *mockFinishSessionRepo) UpdateStatus(ctx context.Context, sessionID uuid.UUID, status string, finishedAt *time.Time, notes string) (bool, error) {
...
func (m *mockFinishSessionRepo) GetCompletedSessionsByUserAndDateRange(_ context.Context, _ uuid.UUID, _, _ time.Time) ([]entities.Session, error) {
	return nil, nil
}

func (m *mockFinishSessionRepo) GetStatsByUserAndPeriod(_ context.Context, _ uuid.UUID, _, _ time.Time) (*ports.SessionStats, error) {
	return &ports.SessionStats{}, nil
}

func (m *mockFinishSessionRepo) GetFrequencyByUserAndPeriod(_ context.Context, _ uuid.UUID, _, _ time.Time) ([]ports.FrequencyData, error) {
	return nil, nil
}

func (m *mockFinishSessionRepo) GetSessionsForStreak(_ context.Context, _ uuid.UUID) ([]time.Time, error) {
	return nil, nil
}
